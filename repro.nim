## Reprobuild dev env for codetracer-cardano-recorder.
##
...
##
## Cardano-specific note: the test corpus consists of pre-compiled
## ``.uplc`` and ``.plutus`` fixtures, so neither aiken nor the
## Cardano node runtime is in ``uses:``. If a future test grows a
## live aiken-compile step, declare aiken here (and add it to the
