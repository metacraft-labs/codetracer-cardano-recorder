...
//!
//! These tests parse and evaluate real Aiken program files through the
...
/// Helper: run the tracer on an Aiken source file and return the output directory.
...
// ---------------------------------------------------------------------------
// Test 1: Record flow_test.ak, verify 3-file output
// ---------------------------------------------------------------------------
...
fn test_aiken_step_events() {
    ...
    // The test block's bare expr triggers compute() which adds its own steps.
    // At minimum: 6 steps in compute + 1 step in flow_test = 7.
    assert!(
        step_events.len() >= 6,
        "should have at least 6 step events for flow_test.ak, got {}",
        step_events.len()
    );
    ...
    assert!(
        ...
        "step events should include line 2 (let a), got lines: {:?}",
        step_lines
    );
    // Line 3: let b = 32
    assert!(
        step_lines.contains(&3),
        "step events should include line 3 (let b), got lines: {:?}",
        step_lines
    );
    // Line 4: let sum_val = a + b
    assert!(
        step_lines.contains(&4),
        "step events should include line 4 (let sum_val), got lines: {:?}",
        step_lines
    );
    // Line 5: let doubled = sum_val * 2
    assert!(
        step_lines.contains(&5),
        "step events should include line 5 (let doubled), got lines: {:?}",
        step_lines
    );
    // Line 6: let final_result = doubled + a
    assert!(
        step_lines.contains(&6),
        "step events should include line 6 (let final_result), got lines: {:?}",
        step_lines
    );
}
...
fn test_aiken_function_calls() {
    ...
    assert!(
        call_count >= 2,
        "trace should contain at least 2 Call events (flow_test + compute), got {}",
        call_count
    );
...
fn test_aiken_metadata_structure() {
    ...
    assert!(
        program_str.contains("flow_test.ak"),
        "metadata 'program' should reference the aiken source file, got: {}",
        program_str
    );
...
// ---------------------------------------------------------------------------
// Test 8: Verify the UPLC CEK machine evaluates programs correctly
// ---------------------------------------------------------------------------

#[test]
fn test_uplc_cek_machine_addition() {
    // Build UPLC: addInteger(10, 32) = 42
...
// ---------------------------------------------------------------------------
// Test 9: Verify UPLC CEK machine computes the full flow_test chain
// ---------------------------------------------------------------------------

#[test]
fn test_uplc_cek_machine_full_computation() {
    // Build the full UPLC computation chain:
    // final_result = (addInteger(10, 32) * 2) + 10 = (42 * 2) + 10 = 84 + 10 = 94
    // Then: equalsInteger(final_result, 94) = True

    // sum_val = addInteger(10, 32)
    ...
    assert_eq!(
        ...
        "UPLC CEK machine should evaluate the full computation chain to True"
    );
}

// ---------------------------------------------------------------------------
// Test 10: Parse and evaluate a UPLC program from text
// ---------------------------------------------------------------------------

#[test]
fn test_uplc_parse_and_eval() {
    // Parse UPLC text for addInteger(10, 32)
    ...
    // Convert to NamedDeBruijn for evaluation.
    let named_db: Program<NamedDeBruijn> = parsed
        .to_named_debruijn()
        .expect("should convert to named debruijn");
    ...
    assert_eq!(
        result_term,
        Term::Constant(Rc::new(Constant::Integer(BigInt::from(42)))),
        "Parsed UPLC program should evaluate to 42"
    );
}

// ---------------------------------------------------------------------------
// Test 11: Parse the UPLC test file and verify evaluation
// ---------------------------------------------------------------------------
...
fn test_uplc_file_evaluation() {
    ...
    // The program checks equalsInteger(94, 94) which should be True.
    assert_eq!(
        result_term,
        Term::Constant(Rc::new(Constant::Bool(true))),
        "flow_test.uplc should evaluate to True (94 == 94)"
    );
}
